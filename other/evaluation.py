...
def gen_train_facts(data_file_name, truth_dir):
    ...
    for data in ori_data:
        ...
        for label in data['labels']:
            rel = label['r']
            for n1 in vertexSet[int(label['h'])]:
                for n2 in vertexSet[int(label['t'])]:
...
if __name__ == '__main__':
    data_list = load_json_file('result/docred_dev_ft_k20.json')

    re_f1, evi_f1, re_f1_ignore_train_annotated, re_f1_ignore_train, re_p, re_r= official_evaluate(data_list, "../data/docred/")

    print("re_f1", re_f1)
    print("evi_f1", evi_f1)
    print("re_f1_ignore_train_annotated", re_f1_ignore_train_annotated)
    print("re_p", re_p)
    print("re_r", re_r)