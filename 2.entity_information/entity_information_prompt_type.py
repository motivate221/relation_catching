...
for data in jsonl_data:
    ...
    if entity_t_name not in ok_entitys_dict[title]:
        ok_entitys_dict[title][entity_t_name] = ""

    ok_entitys_dict[title][entity_h_name] = "ok"
    ok_entitys_dict[title][entity_t_name] = "ok"
...
for doc_id in range(start, end):
    ...
    for index_1 in range(len(entitys_list)):
        entity = entitys_list[index_1]

        if title not in ok_entitys_dict:
            ok_entitys_dict[title] = {}
        if entity not in ok_entitys_dict[title]:
            ok_entitys_dict[title][entity] = ""

        if ok_entitys_dict[title][entity] != "ok":
            continue
...
jsonl_file = f"../data/entity_information_prompt/{data_name}/prompt_{doc_name}_{data_name}_entity_information_doc0-{docred_len}-type.jsonl"
