...
def run_one(prompt):
    result_list = request_model([prompt], temperature=0.9, max_new_tokens=100)
    if not result_list:
        return ""
...
def run_list(prompt_list):
    result_list = request_model(prompt_list, temperature=0.9, max_new_tokens=100)
    response_list_run = []
...
for id in range(start, end):
    if len(prompt_list) == batch_size:
        ...
        for i in range(batch_size):
            ...
            if i >= len(response_list):
                jsonl_data[now_id]["entities_description"] = ""
                print(f"{now_id} data query failed")
            else:
                response = response_list[i]
                jsonl_data[now_id]["entities_description"] = response
    ...
    if save_cnt == 200:
        save_name = f"../data/relation_summary_run/{data_name}/result_{doc_name}_{data_name}_relation_summary_{range_tag}_{save_id}.jsonl"
